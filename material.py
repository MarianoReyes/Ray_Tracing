...
class Material:
    def __init__(self, diffuse=color(255, 255, 255), albedo=(1, 0, 0, 0), spec=0, refractive_index=0, path=None):
...
class Texture:
    ...
    def get_color(self, tx, ty):
        try:
            ...
            x = int(max(min(tx, self.width), 0))
            y = int(max(min(ty, self.height), 0))
